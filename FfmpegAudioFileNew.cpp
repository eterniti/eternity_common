...
bool FfmpegAudioFile::LoadFromFile(const std::string &path, bool show_error)
{
    ...
    int stream_id = av_find_best_stream(format_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
    if (stream_id < 0) 
	{
        if (show_error)
    ...
    AVStream* stream = format_ctx->streams[stream_id];
    ...
    num_channels = codec_ctx->ch_layout.nb_channels;
    ...
    while (av_read_frame(format_ctx, packet) >= 0) 
	{
        if (packet->stream_index == stream_id) 
		{
            if (avcodec_send_packet(codec_ctx, packet) == 0) 
			{
				...
				{
                    int plane_size = av_samples_get_buffer_size(
                        nullptr,
                    ...
                    switch (codec_ctx->sample_fmt) 
					{
                        ...
                        case AV_SAMPLE_FMT_FLT:
                            if (!decoded->Write(frame->extended_data[0], plane_size)) 
							{
                                goto cleanup;
                            }
                            break;
						...
						{
                            int size = num_channels * frame->nb_samples;
                            uint8_t* samples = new uint8_t[size];
                            uint8_t* ptr = samples;
                            for (int nb = 0; nb < frame->nb_samples; nb++) 
							{
						...
						{
                            int size = num_channels * frame->nb_samples * sizeof(int16_t);
                            int16_t* samples = new int16_t[size / sizeof(int16_t)];
                            int16_t* ptr = samples;
							...
							for (int nb = 0; nb < frame->nb_samples; nb++) 
							{
                                for (int ch = 0; ch < num_channels; ch++) 
								{
                                    *(ptr++) = ((int16_t*)frame->extended_data[ch])[nb];
                                }
                            }
                            if (!decoded->Write(samples, size)) {
                                delete[] samples;
						...
						{
                            int size = num_channels * frame->nb_samples * sizeof(uint32_t);
